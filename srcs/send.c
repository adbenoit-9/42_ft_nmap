...
/*   Updated: 2022/08/07 22:23:41 by leon             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int32_t					scan_port(t_ip *ip, t_port *port, t_opt *opt,
...
int32_t					scan_ip(t_ip *ip, t_opt *opt)
{
	uint8_t			*tcp_buf;
	uint8_t			*udp_buf;
	uint8_t			lock_ip = 1;
	...
	else
	{
		...
		while (head && ret == FT_NMAP_OK)
		{
			if (!(((t_port*)head->content)->lock))
			{
				...
				lock_ip = 0;
				if (opt->scans & NMAP_SCANS_FLAG_ANY_TCP)
				{
					ret = build_tcp_ip_raw(&tcp_buf);
				}
				if (ret == FT_NMAP_OK && opt->scans & NMAP_SCANS_FLAG_UDP)
				{
