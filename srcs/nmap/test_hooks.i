...
# 3 "nmap/test_hooks.c" 2


int print_st(t_nmap_setting *root, t_nmap_link *link)
{
 (void)root;
 fprintf(__stderrp, "%s:%d link->sock.ss_family = %08x\n", __func__, 8,
     link->sock.ss_family);
 return (0);
}
...
int print_nd(t_nmap_setting *root, t_nmap_link *link, t_nmap_app *app)
{
 (void)root;
 (void)link;
 fprintf(__stderrp, "%s:%d link->socket = %08x\n", __func__, 18,
     app->socket);
 fprintf(__stderrp, "%s:%d link->port = %04x\n", __func__, 20,
     app->port);
 return (0);
}
...
int print_rd(t_nmap_setting *root, t_nmap_link *link, t_nmap_app *app, t_nmap_scan *scan)
{
 ...
 (void)link;
 (void)app;
 fprintf(__stderrp, "%s:%d link->tcpflag = %02x\n", __func__, 31,
     scan->tcpflag);
 return (0);
}
