...
# 1 "./nmap/mapy_data.h" 1
# 17 "./nmap/mapy_data.h"
# 1 "./buildy/data_buildy.h" 1
# 16 "./buildy/data_buildy.h"
# 1 "./buildy/export_buildy.h" 1
...
# 17 "./buildy/data_buildy.h" 2





t_func_mapy _buildy_exec[8] = {
  build_ipv4_tcp,
  ((void *)0),
  ((void *)0),
  ((void *)0),
...
int exey_wrapper(t_root *root, t_st *st, t_nd *nd, t_rd *rd, int index)
{
 ...
 while (i < 10 && ((rd->exe.tasks[i] & 0x80000000) == 0))
 {
  ++i;
 }
 if (i == 10)
 ...
 if (r == 0)
 {
  ...
  rd->exe.tasks[i] &= ~0x80000000;
  r = 3;

  fprintf(__stderrp, "%s:%d task=%d hook=%d \n", __func__, 48,
  ...
  while (i < root->st_nb)
  {
   ...
   while (j < root->nd_nb)
   {
    ...
    while (k < root->rd_nb)
    {
     ...
     fprintf(__stderrp, "%s:%d i=%d j=%d k=%d\n", __func__, 86, i, j, k);
...
# 147 "mapy/mapy.c"
     r = (*f)
        ...
        (t_nmap_link*)&root->st[i].client,
        (t_nmap_app*)&root->st[i].nd[j].client,
        (t_nmap_scan*)&root->st[i].nd[j].rd[k].client);
     k++;
    }
    j++;
   }
