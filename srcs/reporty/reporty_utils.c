...
/*   Updated: 2022/09/20 11:36:05 by adbenoit         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int dns(void *sockaddr, char *ipstr, char *dest)
{
    ...
    if (sf == AF_INET6) {
        ...
        if (!inet_ntop(sf, &((struct sockaddr_in6 *)sockaddr)->sin6_addr, dest, len)) {
        ...
        if (!inet_ntop(sf, &((struct sockaddr_in *)sockaddr)->sin_addr, dest, len)) {
            ft_strcpy(dest, ipstr);
            perror("inet_ntop");
            ret = REPORTY_ERROR;
        }
