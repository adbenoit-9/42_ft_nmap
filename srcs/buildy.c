...
/*   Updated: 2022/08/16 01:35:54 by leon             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#include "buildy_ip.h"
#include "buildy_tcp.h"
#include "buildy_udp.h"
...
//		memcpy(&buf[20], template_tcp, 20);
		...
		if (r == BUILDY_OK)
		{
			...
			if (((t_scan*)conf_exec)->tcpflag == FLAG_S_SYN)
			{
	   			length += 4;
				tcpoff++;
	   			SET_TCP_DATA(&buf[sizeof(struct iphdr)], syn_mss, 4);
			}
			SET_TCP_FLAGS(&buf[sizeof(struct iphdr)], ((t_scan*)conf_exec)->tcpflag); // TODO 
			SET_TCP_DPORT(&buf[sizeof(struct iphdr)], htons(((t_app*)conf_nd)->port));
			...
			((t_scan*)conf_exec)->packet_length = length;
		}
	}
	return (r);
}
